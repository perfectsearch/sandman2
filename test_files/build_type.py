...
def get_natural_platform_variant():
    ...
    if _defpv is None:
        ...
        if _defpv == 'osx':
            ...
            if bi.bitness == '32' and not bi.arch.startswith('arm'):
                _defpv += '_i686'
            elif bi.bitness == '32' and bi.arch.startswith('armv6'):
                _defpv += "_armv6"
            elif bi.bitness == '32' and bi.arch.startswith('armv7'):
                _defpv += "_armv6"
            elif bi.bitness == '64' and not bi.arch.startswith('arm'):
                _defpv += '_x86-64'
            else:
                _defpv += '_aarm64'
...
_KNOWN_PVs = ['win_x64', 'win_32', 'linux_x86-64', 'linux_i686', 'linux_armv6', 'linux_armv7', 'osx_universal']
...
def fuzzy_match_platform_variant(pv):
    ...
    if '64' in pv:
        bitness = 64
    elif '32' in pv or '86' in pv or 'arm' in pv:
        bitness = 32
    else:
        bitness = None
    if pv.startswith('w') or 'win' in pv:
        ...
        if bitness == 64:
            ...
            if pv.lower().find('arm') == -1:
                ...
                if pv.lower().find('armv6'):
                    return 'linux_armv6'
                elif pv.lower().find('armv7'):
                    return 'linux_armv6'
...
class BuildInfo:
    ...
    def __init__(self):
        self.os = UNAME[0]
        self.host = _clean_host_name(UNAME[1].lower())
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == '--version-stamp':
            print(descr.stamp)
        elif sys.argv[1] == '--build-info':
            print(str(descr))
    else:
        print("built.%s" % get_natural_platform_variant())
