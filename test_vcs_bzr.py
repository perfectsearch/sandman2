...
class TestBzr(unittest.TestCase):
    ...
    def get_repo():
        ...
        try:
            ...
            subprocess.call(['bzr', 'whoami', 'Test User <test@example.com>'])
            subprocess.call(["bzr", "add", "test.txt"])
            # Allow pushing to this repo, normally we would expect this repo to be a bare repository
            #TODO: See if bzr needs something like recieve.denyCurrentBranch
            #subprocess.call(["git", "config", "receive.denyCurrentBranch", "ignore"])
            subprocess.call(["bzr", "commit", "-m", "test comment"])
            return VcsRepo.get_repo(path=config_dir.name, provider='bzr', source=source_dir.name)
        finally:
            os.chdir(cwd)
    ...
    def test_update(self):
        ...
        with open(tmp_file, 'a'):
            os.utime(tmp_file, None)
        os.chdir(source_dir.name)
        subprocess.call(["bzr", "add", "test2.txt"])
        subprocess.call(["bzr", "commit", "-m", "'test comment'"])
        repo.update()
    ...
    def test_publish(self):
        ...
        os.chdir(source_dir.name)
        subprocess.call(["bzr", "revert"])
        assert os.path.exists(os.path.join(source_dir.name, 'test2.txt'))

    def test_shallow_publish(self):
        repo = VcsRepo.get_repo(path=config_dir.name, provider='bzr', source=source_dir.name, type_='built')
        repo.init()
        repo.publish_prep()
        tmp_file = os.path.join(config_dir.name, 'test2.txt')
        with open(tmp_file, 'a'):
    ...
    # Not sure how to do branches within same directory
    @nottest
    ...
    def test_remote_head_revision(self):
        ...
        assert rev_local
        assert rev_local.split('=')[2] == rev_remote
